...
def get_recommendations(req: RecommendationRequest):
    ...
    try:
        ...
        # Degraded: return cached or rule-based
        return RecommendationResponse(
            recommendations=rule_based_fallback(req),
...
def call_claude(req: RecommendationRequest) -> list[dict]:
    ...
    # Build concise event list for context (avoid token explosion bug)
    events_summary = []
    for e in req.available_events[:20]:  # cap at 20
        events_summary.append(
            f"- [{e['id']}] {e['title']} ({e['category']}) on {e['date'][:10]} "
            f"at {e['venue']} — £{e.get('price_gbp', 0) or 0}"
        )

    prompt = f"""You are a personalised event recommendation engine for {DEMO_CITY}.
...
def rule_based_fallback(req: RecommendationRequest) -> list[dict]:
    ...
    # Filter to preferred categories, exclude saved
    candidates = [
        e for e in req.available_events
        if e["id"] not in saved_ids
    ]
