#!/usr/bin/env python3
"""
Sync events into the PULSE events table.

Sources:
  London → Ticketmaster Discovery API  (TICKETMASTER_API_KEY required)
  Paris  → Paris Open Data "que-faire-a-paris" (no auth)
  Other  → Ticketmaster with city= param; Gemini fallback if TM returns nothing

Direct DB mode (K8s CronJob — when POSTGRES_HOST is set):
  Writes directly to PostgreSQL via psycopg2. Also refreshes any extra cities
  already in the DB (from on-demand user requests).
...
# ---------------------------------------------------------------------------
# Ticketmaster — London (lat/lon)
# ---------------------------------------------------------------------------

def fetch_tm_segment_geo(lat: str, lon: str, segment: dict, size: int) -> list[dict]:
    params = {
        "apikey":             TM_API_KEY,
        "geoPoint":           f"{lat},{lon}",
        "radius":             20,
...
def fetch_tm_segment_city(city: str, segment: dict, size: int) -> list[dict]:
    params = {
        "apikey":             TM_API_KEY,
        "city":               city,
        "size":               size,
...
def fetch_london() -> list[dict]:
    ...
    for seg in TM_SEGMENTS:
        ...
        try:
            raw = fetch_tm_segment_geo(lat, lon, seg, seg["count"] * 2)
...
def fetch_paris_segment(tags: list[str], pulse_category: str, count: int) -> list[dict]:
    ...
    params = {"limit": count * 2, "where": where, "order_by": "date_start"}
...
# ---------------------------------------------------------------------------
# Generic city — Ticketmaster with city= param, Gemini fallback
# ---------------------------------------------------------------------------

def fetch_city(city: str) -> list[dict]:
    if TM_API_KEY:
        print(f"-- {city} / Ticketmaster...", file=sys.stderr)
        result, seen = [], set()
        for seg in TM_SEGMENTS_GENERIC:
            try:
                raw = fetch_tm_segment_city(city, seg, seg["count"] * 2)
            except Exception as e:
                print(f"--   WARN: {e}", file=sys.stderr)
                continue
            added = 0
            for ev in raw:
                if added >= seg["count"]:
                    break
                mapped = map_tm_event(ev, city, seg["pulse"])
                if mapped and mapped["id"] not in seen:
                    seen.add(mapped["id"])
                    result.append(mapped)
                    added += 1
        if result:
            print(f"-- {city} total: {len(result)} (Ticketmaster)", file=sys.stderr)
            return result
        print(f"-- {city}: TM returned nothing, falling back to Gemini", file=sys.stderr)

    return fetch_city_gemini(city)


def fetch_city_gemini(city: str) -> list[dict]:
...
id format: {prefix}-<category>-<1..4>. date must be ISO 8601 UTC. price_gbp numeric."""
    ...
    print(f"-- {city} total: {len(events)} (Gemini)", file=sys.stderr)
    return events
...
def emit_sql(events: list[dict]) -> None:
    ...
    for ev in events:
        ...
        print(
            ...
            f"'{sq(ev['id'])}', '{sq(ev['title'])}', '{sq(ev['description'])}', "
            f"'{sq(ev['category'])}', '{sq(ev['venue'])}', '{sq(ev['address'])}', "
            f"'{sq(ev['city'])}', '{ev['date']}', {ev['price_gbp']}, "
            f"'{sq(ev['image_url'])}', '{sq(ev['ticket_url'])}', {tags}"
            f") ON CONFLICT (id) DO UPDATE SET "
            f"title=EXCLUDED.title, description=EXCLUDED.description, category=EXCLUDED.category, "
            f"venue=EXCLUDED.venue, address=EXCLUDED.address, city=EXCLUDED.city, "
    ...
    print()
    print("COMMIT;")
    print(f"\\echo 'Synced {len(events)} events'")
...
def get_extra_cities(conn) -> list[str]:
    cur = conn.cursor()
    cur.execute("SELECT DISTINCT city FROM events WHERE city NOT IN ('London', 'Paris')")
...
def main() -> None:
    ...
    # CronJob: also refresh any extra cities users have requested on-demand
    if os.getenv("POSTGRES_HOST"):
        ...
        try:
            ...
            extra = get_extra_cities(conn)
            conn.close()
            for city in extra:
                print(f"-- Refreshing extra city: {city}", file=sys.stderr)
                events += fetch_city(city)
        except Exception as e:
            print(f"-- WARN: could not load extra cities: {e}", file=sys.stderr)

    print(f"-- Grand total: {len(events)} events", file=sys.stderr)
    if not events:
        print("-- ERROR: no events fetched — aborting", file=sys.stderr)
        sys.exit(1)
